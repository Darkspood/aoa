...
\subsection{Greedy vs. Local Search Illustration}
Figure~\ref{fig:flow} gives a schematic branch-style example of how the two methods behave on the same instance. On the left, greedy opens facilities sequentially and keeps its early placement decisions fixed. On the right, local search starts from the greedy solution and then improves it by swapping open and closed facilities until no improving one-swap move remains.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/greedy_vs_local_search_flow.png}
    \caption{Schematic comparison between greedy construction and local-search improvement on a small branching example.}
    \label{fig:flow}
\end{figure}
...
These are worst-case upper bounds. The nearest-facility bookkeeping and early stopping used in the implementation reduce practical work per evaluated swap and improve constant factors on many instances, but they do not change the asymptotic worst-case bounds stated above. This complexity discussion explains the empirical trend seen in the runtime plots: local search is more expensive than the greedy baseline, but the number of improving passes remains fairly small for the tested instances, which keeps the overall runtime manageable.
...
At size 60, the average improvement dropped to 2.24\%. This is still a real and consistent gain, but with only five random seeds per size it is safer to interpret this dip as possible variability rather than as a strong structural pattern. Approximation heuristics should therefore be evaluated empirically rather than judged only from a small number of examples.
...
The recorded pass count includes the final pass in which the algorithm scans the neighborhood and confirms that no improving swap remains. Since the implementation applies exactly one best improving swap per improving pass and the final pass is non-improving, the number of improving swaps equals the number of passes minus one in the reported experiments.

The number of evaluated swaps also matches the expected scaling behavior. For instance, when the instance size is 100 with $k=20$, each pass has an upper bound of $20 \times 80 = 1600$ possible swaps. Since the average number of passes is 8.00, the corresponding full-enumeration count would be 12800, which matches the recorded average in this data. This consistency is a useful sanity check for the implementation, even though the early-stopping logic can reduce the work done within individual swap evaluations.
...
Table~\ref{tab:detailed} lists every trial used in the evaluation. Including the full table in the results section makes it easy to verify that, in our experiments, local search never performed worse than the greedy baseline on the tested instances.
...
\bottomrule
\endlastfoot
\input{results/detailed_results.tex}
\end{longtable}
