...
The challenge comes from the combinatorial number of possible subsets $S$. Even when facility and client sets are modest in size, the number of candidate solutions grows quickly as $\binom{|F|}{k}$. Exhaustive search becomes impractical, so exact optimization is expensive for larger instances. This motivates the use of approximation algorithms and heuristics that provide strong solutions at much lower computational cost.
...
Let $m = |F|$ and $n = |C|$. In the greedy baseline, each of the $k$ selection rounds evaluates up to $m$ candidate additions, and each candidate addition scans all clients. In the present implementation, the worst-case time is roughly $O(kmn)$.

For local search, each pass considers $k(m-k)$ candidate swaps. Each candidate swap may inspect all $n$ clients, although the implementation includes early stopping when a partial cost already exceeds the current best known swap cost. The per-pass worst-case complexity is therefore about $O(k(m-k)n)$. If the algorithm converges after $T$ passes, the total local-search effort is about $O(Tk(m-k)n)$.

This complexity explains the empirical trend seen in the runtime plots: local search is more expensive than the greedy baseline, but the number of improving passes remains fairly small for the tested instances, which keeps the overall runtime manageable.
...
The generated results were checked directly after the experiments completed. In all 25 trials, local search produced a cost that was less than or equal to the greedy baseline cost. There were no violations of the expected monotonicity property. More specifically:
\begin{itemize}
...
At sizes 80 and 100, the average gains returned to around 4\%. The runtime grew noticeably, especially at size 100 where the average total runtime reached about 0.5262 seconds. Even so, the absolute runtime remained small enough for experimental use on these moderate-scale instances.
...
The recorded pass count includes the final pass in which the algorithm scans the neighborhood and confirms that no improving swap remains. Therefore, the number of improving swaps is usually one less than the number of passes, except in tie cases or when multiple improvements occur before the final termination pass.
